...
def run_capture_replay(args: argparse.Namespace) -> None:
    ...
    When ``--mask-dir`` is also provided, mask EXRs are loaded and sent
    in lockstep on the mask channel.
    """
    ...
    pool_workers = num_cameras + (1 if send_masks else 0)
        ...
        while True:
            ...
            for chunk_idx, (chunk_start, chunk_end) in enumerate(chunks):
                ...
                for local_idx in range(chunk_size):
                    ...
                    total_sent += num_cameras + (1 if send_masks else 0)
                    frame_count += 1
...
def main() -> None:
    ...
    if args.synthetic:
        if args.capture_dir or args.mask_dir:
            print("ERROR: --synthetic cannot be combined with --capture-dir or --mask-dir")
            sys.exit(1)
        run_synthetic(args)
    elif args.capture_dir or args.mask_dir:
        if args.capture_dir and not args.mask_dir:
            run_capture_replay(args)
        elif args.mask_dir and not args.capture_dir:
